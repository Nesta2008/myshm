...
// Инициализация блока PLL
  ...
  SystemCoreClockUpdate();   //Обновление значения тактовой частоты
}
...
void CLK_TIMER1 (void)              //тактирование таймера 1
{
 ...
 // MDR_RST_CLK->PER_CLOCK |= (1 << RST_CLK_PER_CLOCK_PCLK_EN_TIMER1_Pos);  //включение периферийного блока таймера
 // MDR_RST_CLK->TIM_CLOCK |= (1 << RST_CLK_TIM_CLOCK_TIM1_CLK_EN_Pos);     //разрешение на тактирование периферийного блока таймера
}
