...
void TIM_Init (void)      // функция настройки таймера по уроку scillbox
{
  ...
  Tim_CntInitStruct.TIMER_ARR_UpdateMode = TIMER_ARR_Update_Immediately;          //Режим сброса счетчика   // Регистр ARR можно обновлять в любое время.
  Tim_CntInitStruct.TIMER_BRK_Polarity = TIMER_BRKPolarity_NonInverted;   //Полярность выхода
  Tim_CntInitStruct.TIMER_ETR_FilterConf = TIMER_Filter_1FF_at_TIMER_CLK;  //Сигнал зафиксирован в 1-м триггере на частоте TIM_CLK (В нашем случае оставляем по-умолчанию).
  Tim_CntInitStruct.TIMER_ETR_Polarity = TIMER_ETRPolarity_NonInverted;// ETR на входе не инвертируется (мы его и не используем)
  Tim_CntInitStruct.TIMER_ETR_Prescaler = TIMER_ETR_Prescaler_None;// Частота ETR на входе не делится (ETR не используем.).
  ...
  TIMER_ITConfig (MDR_TIMER1, TIMER_STATUS_CNT_ZERO, ENABLE); // Разрешаем прерывание
...
void TIM_Init_CMSIS (void)      // функция настройки таймера
{
  ...
  */
  ...
  MDR_TIMER1->PSG = (800 - 1);     // Предделитель тактовой частоты. Данный регистр отвечает за деление входного сигнала

  // Период перезагрузки;
  // значение выбрано для периода в 1 секунду по формуле:
  // Tr = (PSG / CLK) * (ARR + 1)
  MDR_TIMER1->CNT=0;              // Начальное значение счета  Считай с 0
  /*   Регистр CNT можно назвать основой. Именно значение в нем сравнивается с «эталонным» и в случае 
  совпадения происходит какое-либо действие. Именно с него таймер начинает считать.
   В нашем случае достаточно, чтобы он был равен нулю. Несмотря на то, что он при включении и так должен 
   был быть равен нулю, на всякий случай лучше сбросить его, 
   т.к. возможно, что после программной перезагрузки значение в нем будет не ноль.
  */
  
  
  MDR_TIMER1->ARR = 8000;       //Именно с этим числом будет сравниваться число в CNT
